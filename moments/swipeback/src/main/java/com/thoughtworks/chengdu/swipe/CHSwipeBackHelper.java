 ...
 */
package com.thoughtworks.chengdu.swipe;
 ...
 * 描述:滑动返回帮助类
 */
public class CHSwipeBackHelper {
    private Activity mActivity;
    private Delegate mDelegate;
    private CHSwipeBackLayout mSwipeBackLayout;
    ...
    public static void init(Application application, List<Class<? extends View>> problemViewClassList) {
        CHSwipeBackManager.getInstance().init(application, problemViewClassList);
    }
     ...
     * @param delegate
     */
    public CHSwipeBackHelper(Activity activity, Delegate delegate) {
        mActivity = activity;
        mDelegate = delegate;
    ...
    private void initSwipeBackFinish() {
        if (mDelegate.isSupportSwipeBack()) {
            mSwipeBackLayout = new CHSwipeBackLayout(mActivity);
            mSwipeBackLayout.attachToActivity(mActivity);
            mSwipeBackLayout.setPanelSlideListener(new CHSwipeBackLayout.PanelSlideListener() {
                @Override
                public void onPanelSlide(View panel, float slideOffset) {
                    // 开始滑动返回时关闭软键盘
                    if (slideOffset < 0.03) {
                        CHKeyboardUtil.closeKeyboard(mActivity);
                    }
     ...
     * @return
     */
    public CHSwipeBackHelper setSwipeBackEnable(boolean swipeBackEnable) {
     ...
     * @return
     */
    public CHSwipeBackHelper setIsOnlyTrackingLeftEdge(boolean isOnlyTrackingLeftEdge) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CHSwipeBackHelper setIsWeChatStyle(boolean isWeChatStyle) {
        if (mSwipeBackLayout != null) {
    ...
    /**
     * 设置阴影资源 id。默认值为 R.drawable.CH_sbl_shadow
     *
     * @param shadowResId
     * @return
     */
    public CHSwipeBackHelper setShadowResId(@DrawableRes int shadowResId) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CHSwipeBackHelper setIsNeedShowShadow(boolean isNeedShowShadow) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public CHSwipeBackHelper setIsShadowAlphaGradient(boolean isShadowAlphaGradient) {
        if (mSwipeBackLayout != null) {
     ...
     * @param threshold
     */
    public CHSwipeBackHelper setSwipeBackThreshold(@FloatRange(from = 0.0f, to = 1.0f) float threshold) {
        if (mSwipeBackLayout != null) {
     ...
     * @param overlap
     */
    public CHSwipeBackHelper setIsNavigationBarOverlap(boolean overlap) {
        if (mSwipeBackLayout != null) {
    ...
    public static void executeForwardAnim(Activity activity) {
        activity.overridePendingTransition(R.anim.ch_sbl_activity_forward_enter, R.anim.ch_sbl_activity_forward_exit);
    }
    ...
    public static void executeBackwardAnim(Activity activity) {
        activity.overridePendingTransition(R.anim.ch_sbl_activity_backward_enter, R.anim.ch_sbl_activity_backward_exit);
    }
    ...
    public static void executeSwipeBackAnim(Activity activity) {
        activity.overridePendingTransition(R.anim.ch_sbl_activity_swipeback_enter, R.anim.ch_sbl_activity_swipeback_exit);
    }
