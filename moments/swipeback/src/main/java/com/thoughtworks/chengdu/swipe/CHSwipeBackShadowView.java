 ...
 * 描述:左侧阴影控件
 */
class CHSwipeBackShadowView extends FrameLayout {
    private static final String TAG = CHSwipeBackShadowView.class.getSimpleName();
     ...
     * 阴影资源 id
     */
    private int mShadowResId = R.drawable.ch_sbl_shadow;
    /**
     * 阴影区域的透明度是否根据滑动的距离渐变
     */
    ...
    CHSwipeBackShadowView(Activity activity) {
        super(activity);
        mActivity = activity;
    ...
    void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            Activity preActivity = CHSwipeBackManager.getInstance().getPenultimateActivity(mActivity);
            if (preActivity != null) {
    ...
    private boolean containsProblemView(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (CHSwipeBackManager.getInstance().isProblemView(childView)) {
                return true;
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            Activity preActivity = CHSwipeBackManager.getInstance().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
